...
extern unsigned int                     Idle_Flags[CFG_NU_OS_KERN_PLUS_SMP_CPU_COUNT];

#else

extern unsigned int                     Idle_Flags[1];

#endif /* CFG_NU_OS_KERN_PLUS_SMP_ENABLE */

/* Check if target is multi-core */
#define GET_CPU_ID()                    ESAL_GE_CPU_ID_GET()
...
/* SMP */
#define SMP_TASK_AFFIN_CHANGE_EID       (KRN_EID_BASE + 149)
#define SMP_HISR_AFFIN_CHANGE_EID       (KRN_EID_BASE + 150)
#define SMP_INT_AFFIN_CHANGE_EID        (KRN_EID_BASE + 151)
#define SMP_BCD_CPU_CREATE_EID          (KRN_EID_BASE + 152)
#define SMP_BCD_TASK_ADD_EID            (KRN_EID_BASE + 153)
#define SMP_BCD_HISR_ADD_EID            (KRN_EID_BASE + 154)
#define SMP_BCD_IRQ_ADD_EID             (KRN_EID_BASE + 155)
#define SMP_SPINLCK_CREATE_EID          (KRN_EID_BASE + 156)
#define SMP_SPINLCK_DEL_EID             (KRN_EID_BASE + 157)
#define SMP_SPINLCK_OBT_EID             (KRN_EID_BASE + 158)
#define SMP_SPINLCK_IRQ_OBT_EID         (KRN_EID_BASE + 159)
#define SMP_SPINLCK_REL_EID             (KRN_EID_BASE + 160)
#define SMP_SPINLCK_IRQ_REL_EID         (KRN_EID_BASE + 161)
...
#if ((CFG_NU_OS_SVCS_TRACE_CORE_TRACK_TRACE_OVERHEAD == NU_TRUE) || (CFG_NU_OS_SVCS_TRACE_CORE_PC_HOTSPOT_SUPPORT == NU_TRUE))
...
void    Log_U64U64II(unsigned short evt_id, unsigned long long u64_1, unsigned long long u64_2, int int_1, int int_2);

#endif /* ((CFG_NU_OS_SVCS_TRACE_CORE_TRACK_TRACE_OVERHEAD == NU_TRUE) || (CFG_NU_OS_SVCS_TRACE_CORE_PC_HOTSPOT_SUPPORT == NU_TRUE)) */

#if (CFG_NU_OS_SVCS_TRACE_CORE_TRACK_TRACE_OVERHEAD == NU_TRUE)

void    Log_U64U64I(unsigned short evt_id, unsigned long long u64_1, unsigned long long u64_2, int int_1);
...
#define TH_pVII_SIZE                    (TH_SIZE + sizeof(void*) + sizeof(int) + sizeof(int))
#define TH_pVpII_SIZE                   (TH_SIZE + sizeof(void*) + sizeof(int*) + sizeof(int))
...
#define TH_pVpU32I_SIZE					(TH_SIZE + sizeof(void*) + sizeof(unsigned long*) + sizeof(int))
#define TH_IpU32I_SIZE					(TH_SIZE + sizeof(int) + sizeof(unsigned long*) + sizeof(int))
#define TH_IU32I_SIZE					(TH_SIZE + sizeof(int) + sizeof(unsigned long) + sizeof(int))
...
void    Log_pVII(unsigned short evt_id, void* pV, int I, int status);
void    Log_pVpII(unsigned short evt_id, void* pV, int* pI, int status);
...
void    Log_pCI(unsigned short evt_id, const char* pC, int status);
...
void    Log_pCpCI(unsigned short evt_id, const char* pC1, const char* pC2, int fd);
void    Log_pCpCpC(unsigned short evt_id, char* pC1, char* pC2, const char* pC3);
void    Log_pCpCpCU16U16I(unsigned short evt_id, char* pC1, char* pC2, const char* pC3, unsigned short u16_1, unsigned short u16_2, int fd);
void    Log_pCU16U16I(unsigned short evt_id, const char* pC, unsigned short u16_1, unsigned short u16_2, int fd);
...
void    Log_pCU32(unsigned short evt_id, const char* pC, unsigned long u32);
void    Log_pVpU32I(unsigned short evt_id, void* pV, unsigned long* u32, int status);
void    Log_IpU32I(unsigned short evt_id, signed long i32, unsigned long* u32, int status);
void    Log_IU32I(unsigned short evt_id, int i, unsigned long u32, int status);
...
#define    T_TASK_CREATED(p_tcb, p_task, p_stk, p_name, stk_size, time_slice, priority,        \
                                            ...
                                            if(((TC_TCB*)p_tcb)->tc_id == TC_TASK_ID){        \
...
#define    T_TASK_RESET(p_tcb, status)      if(((TC_TCB*)p_tcb)->tc_id == TC_TASK_ID){        \
                                            APPLY_MASK(NU_TRACE_TASKS, TASK_RESET_EID,         \
                                            Log_pVI(TASK_RESET_EID, p_tcb, status))}
#define    T_TASK_READY(p_tcb)              if(((TC_TCB*)p_tcb)->tc_id == TC_TASK_ID){        \
                                            APPLY_MASK(NU_TRACE_TASKS, TASK_READY_EID,         \
                                            Log_pV(TASK_READY_EID, p_tcb))}
#define    T_TASK_RUNNING(p_tcb)            if(((TC_TCB*)p_tcb)->tc_id == TC_TASK_ID){        \
                                            APPLY_MASK(NU_TRACE_TASKS, TASK_RUNIG_EID,         \
                                            Log_pV(TASK_RUNIG_EID, p_tcb))}
#define    T_TASK_SUSPEND(p_tcb, sus_type)  if(((TC_TCB*)p_tcb)->tc_id == TC_TASK_ID){        \
                                            APPLY_MASK(NU_TRACE_TASKS, TASK_SSUSP_EID,         \
                                            Log_pVU32(TASK_SSUSP_EID, p_tcb, sus_type))}
#define    T_TASK_TERMINATED(p_tcb, status) if(((TC_TCB*)p_tcb)->tc_id == TC_TASK_ID){        \
                                            APPLY_MASK(NU_TRACE_TASKS, TASK_TERMT_EID,         \
                                            Log_pVI(TASK_TERMT_EID, p_tcb, status))}
#define    T_TASK_DELETED(p_tcb, status)    if(((TC_TCB*)p_tcb)->tc_id == TC_TASK_ID){        \
                                            APPLY_MASK(NU_TRACE_TASKS, TASK_DELET_EID,         \
...
/* Messages */
/************/
#define    T_MSG_CREATE(id, cb, start, name, size, msg_size, msg_type, susp_type, status)      \
if (id == PI_PIPE_ID)                                                                          \
{                                                                                              \
    T_PIPE_CREATE(cb, start, name, size, msg_size, msg_type, susp_type, status)                \
}                                                                                              \
else                                                                                           \
{                                                                                              \
    T_Q_CREATE(cb, start, name, size, msg_size, msg_type, susp_type, status)                   \
}

#define    T_MSG_RESET(id, cb, status)                                                         \
if (id == PI_PIPE_ID)                                                                          \
{                                                                                              \
    T_PIPE_RESET(cb, status)                                                                   \
}                                                                                              \
else                                                                                           \
{                                                                                              \
    T_Q_RESET(cb, status)                                                                      \
...
#define    T_MSG_BCAST(id, cb, msg, size, susp, status)                                        \
if (id == PI_PIPE_ID)                                                                          \
{                                                                                              \
    T_PIPE_BCAST(cb, msg, size, susp, status)                                                  \
}                                                                                              \
else                                                                                           \
{                                                                                              \
    T_Q_BCAST(cb, msg, size, susp, status)                                                     \
}

#define    T_MSG_SEND(id, cb, msg, size, susp, status)                                         \
if (id == PI_PIPE_ID)                                                                          \
{                                                                                              \
    T_PIPE_SEND(cb, msg, size, susp, status)                                                   \
}                                                                                              \
else                                                                                           \
{                                                                                              \
    T_Q_SEND(cb, msg, size, susp, status)                                                      \
}

#define    T_MSG_SEND2FRONT(id, cb, msg, size, susp, status)                                   \
if (id == PI_PIPE_ID)                                                                          \
{                                                                                              \
     T_PIPE_SEND2FRONT(cb, msg, size, susp, status)                                            \
}                                                                                              \
else                                                                                           \
{                                                                                              \
     T_Q_SEND2FRONT(cb, msg, size, susp, status)                                               \
}

#define    T_MSG_RECV(id, cb, msg, size, a_size, susp, status)                                 \
if (id == PI_PIPE_ID)                                                                          \
{                                                                                              \
     T_PIPE_RECV(cb, msg, size, a_size, susp, status)                                          \
}                                                                                              \
else                                                                                           \
{                                                                                              \
     T_Q_RECV(cb, msg, size, a_size, susp, status)                                             \
}

#define    T_MSG_DELETE(id, cb, status)                                                        \
if (id == PI_PIPE_ID)                                                                          \
{                                                                                              \
     T_PIPE_DELETE(cb, status)                                                                 \
}                                                                                              \
else                                                                                           \
{                                                                                              \
     T_Q_DELETE(cb, status)                                                                    \
}
...
/* Idle */
/********/
#define    T_IDLE_ENTRY()                   Idle_Flags[GET_CPU_ID()] = 1; APPLY_MASK(NU_TRACE_CPU_STATE, IDLE_ENTRY_EID, Log_U32(IDLE_ENTRY_EID, GET_CPU_ID()))
#define    T_IDLE_EXIT()                    if(Idle_Flags[GET_CPU_ID()] == 1) {Idle_Flags[GET_CPU_ID()] = 0; APPLY_MASK(NU_TRACE_CPU_STATE, IDLE_EXIT_EID, Log_U32(IDLE_EXIT_EID, GET_CPU_ID()))}
...
/* Nucleus SMP        */
/**********************/
#define T_TASK_AFFIN_CHG(tcb, affinity_mask, status)						   \
                                            APPLY_MASK(NU_TRACE_SMP_INFO, SMP_TASK_AFFIN_CHANGE_EID,   \
                                            Log_pVpU32I(SMP_TASK_AFFIN_CHANGE_EID, tcb, affinity_mask, status))
#define T_HISR_AFFIN_CHG(hcb, affinity_mask, status)						   \
                                            APPLY_MASK(NU_TRACE_SMP_INFO, SMP_HISR_AFFIN_CHANGE_EID,   \
                                            Log_pVpU32I(SMP_HISR_AFFIN_CHANGE_EID, hcb, affinity_mask, status))
#define T_INT_AFFIN_CHG(vector, affinity_mask, status)                                                 \
                                            APPLY_MASK(NU_TRACE_SMP_INFO, SMP_INT_AFFIN_CHANGE_EID,    \
                                            Log_IpU32I(SMP_INT_AFFIN_CHANGE_EID, vector, affinity_mask, status))
#define T_BCD_CPU_CREATE(cpu_id, status)    APPLY_MASK(NU_TRACE_SMP_INFO, SMP_BCD_CPU_CREATE_EID,      \
                                            Log_U32I(SMP_BCD_CPU_CREATE_EID, cpu_id, status))
#define T_BCD_TASK_ADD(tcb, cpu_id, status) APPLY_MASK(NU_TRACE_SMP_INFO, SMP_BCD_TASK_ADD_EID,        \
                                            Log_pVU32I(SMP_BCD_TASK_ADD_EID, tcb, cpu_id, status))
#define T_BCD_HISR_ADD(hcb, cpu_id, status) APPLY_MASK(NU_TRACE_SMP_INFO, SMP_BCD_HISR_ADD_EID,        \
                                            Log_pVU32I(SMP_BCD_HISR_ADD_EID, hcb, cpu_id, status))
#define T_BCD_IRQ_ADD(vector, cpu_id, status)							   \
                                            APPLY_MASK(NU_TRACE_SMP_INFO, SMP_BCD_IRQ_ADD_EID,         \
                                            Log_IU32I(SMP_BCD_IRQ_ADD_EID, vector, cpu_id, status))
#define T_SPINLCK_CREATE(spinlock, status)  APPLY_MASK(NU_TRACE_SMP_INFO, SMP_SPINLCK_CREATE_EID,      \
                                            Log_pVI(SMP_SPINLCK_CREATE_EID, spinlock, status))
#define T_SPINLCK_DELETE(spinlock, status)  APPLY_MASK(NU_TRACE_SMP_INFO, SMP_SPINLCK_DEL_EID,         \
                                            Log_pVI(SMP_SPINLCK_DEL_EID, spinlock, status))
#define T_SPINLCK_OBTAIN(spinlock, status)  APPLY_MASK(NU_TRACE_SMP_INFO, SMP_SPINLCK_OBT_EID,         \
                                            Log_pVI(SMP_SPINLCK_OBT_EID, spinlock, status))
#define T_SPINLCK_IRQ_OBTAIN(spinlock, irq_state, status)                                              \
                                            APPLY_MASK(NU_TRACE_SMP_INFO, SMP_SPINLCK_IRQ_OBT_EID,     \
                                            Log_pVpII(SMP_SPINLCK_IRQ_OBT_EID, spinlock, irq_state, status))
#define T_SPINLCK_RELEASE(spinlock, status) APPLY_MASK(NU_TRACE_SMP_INFO, SMP_SPINLCK_REL_EID,         \
                                            Log_pVI(SMP_SPINLCK_REL_EID, spinlock, status))
#define T_SPINLCK_IRQ_RELEASE(spinlock, irq_state, status)                                             \
                                            APPLY_MASK(NU_TRACE_SMP_INFO, SMP_SPINLCK_IRQ_REL_EID,     \
                                            Log_pVII(SMP_SPINLCK_IRQ_REL_EID, spinlock, irq_state, status))

/************************************/
/*  Trace marker in else statement  */
/************************************/
#define T_ELSE(trace_marker)    \
        else                    \
        {                       \
            trace_marker        \
        }
...
/************************************/
/*  Trace marker in if statement    */
/************************************/
#define T_IF(_ifcheck, _trace_marker)           \
if (_ifcheck)                                   \
{                                               \
    _trace_marker                               \
}
...
#define T_HISR_AFFIN_CHG(hcb, affinity_mask, status)
#define T_INT_AFFIN_CHG(vector, affinity_mask, status)
#define T_BCD_CPU_CREATE(cpu_id, status)
#define T_BCD_TASK_ADD(tcb, cpu_id, status)
#define T_BCD_HISR_ADD(hcb, cpu_id, status)
#define T_BCD_IRQ_ADD(vector, cpu_id, status)
#define T_SPINLCK_CREATE(spinlock, status)
#define T_SPINLCK_DELETE(spinlock, status)
#define T_SPINLCK_OBTAIN(spinlock, status)
#define T_SPINLCK_IRQ_OBTAIN(spinlock, irq_state, status)
#define T_SPINLCK_RELEASE(spinlock, status)
#define T_SPINLCK_IRQ_RELEASE(spinlock, irq_state, status)
