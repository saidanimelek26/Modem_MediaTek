...
#****************************************************************************/
use strict;
BEGIN{push(@INC,'../../../pcore/tools/', './pcore/tools/', './common/tools/MemoryUtility/')};
use CommonUtility; 
...
my %symbol_located_obj_lib; #{$strSymbolName} => [[strObjName1,strLibName1],[strObjName2,strLibName2],...]
#****************************************************************************
# oo >>>  Finished
#****************************************************************************
return 1;
...
sub ParseMAP
{
    ...
    if(defined $g_MAPPath and -e $g_MAPPath)
    {
        ...
        my ($strObjName, $strLibName, $strInputSectionName, $nInputSectionSize, $bSkipParseSymbol) = ("NOT_FOUND", undef, undef, undef, 0); #since $strPreInputSection will be set back to undef after _ParseContent(), we need another variable to store it
        while (<FILE_HANDLE>)
        {
            my $strLine = $_;
            ...
            elsif($b_enter_reference_table == 0 )
            {
                ...
                {# .debug_aranges
                 #               0x0000000000000000       0x20 ./build/MT6589_MD2_TDD128HSPA/SKYPA/bin/lib/libsys_drv.a(bootarm_gcc.obj)
                    $strPreInputSection = $1; $strInputSectionName = $1; next;
                }
                ($strPreInputSection, $strObjName, $strLibName, $strInputSectionName, $nInputSectionSize, $bSkipParseSymbol) = _ParseContent($strLine, $strPreInputSection, $strPreExeRegion, $strObjName, $strLibName, $strInputSectionName, $nInputSectionSize, $bSkipParseSymbol);
            }
            else
            {
                my $tmp_ref;
...
sub _ParseContent
{
    my ($strInput, $strPreInputSection, $strPreExeRegion, $strObjName, $strLibName, $strInputSectionName, $nInputSectionSize, $bSkipParseSymbol) = @_;
    chomp($strInput);
    ...
    { #  C$$code        0x0000000000000000      0x210 ./build/MT6572_DEMO_TDD128HSPA/DEFAULT/bin/lib/libsys_drv.a(bootarm_gcc.obj)
        ...
        # 0x0000000000000000        0x8 ./build/TK6291_FPGA/BASIC/bin/lib/libdevdrv.a(cc_irq_sys_comm.obj)  <-- skip such case in parse symbol
        if($base =~ /0x0000000000000000/)
        {
            $bSkipParseSymbol = 1;
            $strObjName = "NOT_FOUND" 
        }
        else
        {
            $bSkipParseSymbol = 0;
    ...
    if(!$bSkipParseSymbol)
    {
        ...
        if($strInput =~ /^\s+(0x\w+)\s+(\w+)$/ and $strInput !~/$skip_pattern/)
        {
            # for below case
            #                0x00024054                p_SLILM
            my $strAddress = $1;
            my $strSymbolName = $2;
            $symbol_info{$strSymbolName} = [$strSymbolName, $strAddress, $strObjName, $strLibName, $strPreExeRegion, $strInputSectionName, $nInputSectionSize];
            push(@{$symbol_located_obj_lib{$strSymbolName}}, [$strObjName, $strLibName]);
        }
    }
    return ($strPreInputSection, $strObjName, $strLibName, $strInputSectionName, $nInputSectionSize, $bSkipParseSymbol);
}
...
sub _UpdateRORWZISize
{
    ...
    #neet to skip 
    # .rodata.str1.1
    #            0x0000000000000000        0x3 ./build/MT6589_MD2_TDD128HSPA/SKYPA/bin/lib/libtst.a(tst_spare_ram_logging.obj)
...
{
    my ($strSymbolName) = @_;
    return \@{$symbol_located_obj_lib{$strSymbolName}};
}
