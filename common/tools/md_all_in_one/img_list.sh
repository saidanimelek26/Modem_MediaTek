...
usage() {
  ...
  echo -e "         4. img_list.sh unpack \"single_modem_image\" \"$output_md_file\" \"replace_file=A.bin,B.bin,C.bin\""
  echo -e
  echo -e "Description:"
...
unpack_func () {
  ...
  if [[ "$chk_unpack_file" != "" ]]; then
    echo "Error: Please do not place xxx_unpack file: $chk_unpack_file under the current folder!"
    exit 1
  fi
