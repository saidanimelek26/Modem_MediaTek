...
{   
   ...
   kal_trace( TRACE_GROUP_AUD_PLAYBACK, MCU_DTMF_MCU_PROCESS_INTERNAL_ENTER);
...
////!         PcmSink_Mute(KAL_TRUE, PCMSINK_MUTE_TONE);
   ...
   kal_trace( TRACE_GROUP_AUD_PLAYBACK, MCU_DTMF_MCU_PROCESS_INTERNAL_LEAVE);
   kal_give_enh_mutex(MCUTONEMUTEXID);
   return KAL_FALSE;
}
...
static void dtmf_mcu_Init(kal_uint16 uSampleRate)
{
   ...
   {
      kal_trace( TRACE_GROUP_AUD_PLAYBACK, MCU_DTMF_INIT, uSampleRate, (DTMF_MCU_BUFFER_SIZE<<2), uTotalBufferSize);
   	  ASSERT(0);
   }
