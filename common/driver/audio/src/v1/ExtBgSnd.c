...
#define EXT_BGSND_SRC_BUF_SIZE ((BGSND_BUFFER_LEN) * 10) //buffering 10 frames
...
{	
  kal_trace( TRACE_FUNC, DSP_BGSND_INIT_ENTER);
	memset(&DSP_BGSnd, 0, sizeof(DSP_BKGSND_T));
	//has_ul = has_dl = 0;
	kal_trace( TRACE_FUNC,  DSP_BGSND_INIT_LEAVE);
}

static void DSP_BGSND_ConfigMixer(kal_bool bSPHFlag, kal_int8 SNDGain, BGSND_PROCESS_TYPE type)
{
  kal_trace( TRACE_FUNC, DSP_BGSND_CONFIGMIXER_ENTER);	
  kal_trace( TRACE_FUNC, DSP_BGSND_CONFIGMIXER_BGSND_PROCESS_TYPE, type, bSPHFlag, SNDGain);
	...
	kal_trace( TRACE_FUNC,  DSP_BGSND_CONFIGMIXER_LEAVE);
}

static void DSP_BGSND_UpdateMixer() // private
{
  kal_trace( TRACE_FUNC, DSP_BGSND_UPDATEMIXER_ENTER);	   
  ...
  kal_trace( TRACE_FUNC, DSP_BGSND_UPDATEMIXER_LEAVE);	   
}


void DSP_BGSND_Stop(void)
{
  kal_trace( TRACE_FUNC, DSP_BGSND_STOP_ENTER);
  ...
  kal_trace( TRACE_FUNC, DSP_BGSND_STOP_LEAVE);   
}

#if !defined(DSP_BGS_UP_DOWN_INT_SEPERATE)
void DSP_BGSND_Shared_Hisr_DL_UL(void * param)
{
  kal_trace( TRACE_FUNC, DSP_BGSND_SHARED_HISR_DL_UL_ENTER);   
   DSP_BGSND_Hisr(BGSND_UL_PROCESS);
   DSP_BGSND_Hisr(BGSND_DL_PROCESS);
  kal_trace( TRACE_FUNC, DSP_BGSND_SHARED_HISR_DL_UL_LEAVE);  
}
#endif
/*
...
void DSP_BGSND_Hisr(void * param)
{
   ...
   kal_trace( TRACE_FUNC, DSP_BGSND_HISR_ENTER);   
   
   kal_trace( TRACE_FUNC, DSP_BGSND_HISR_BGSND_PROCESS_TYPE, type, DSP_BGSnd.state);
   ...
   kal_trace( TRACE_FUNC, DSP_BGSND_HISR_LEAVE); 
}

static void DSP_BGSND_Start(void (*Ext_DLHisr)(void), 
                            void (*Ext_ULHisr)(void))
{
   kal_trace( TRACE_FUNC, DSP_BGSND_START_ENTER);
   ...
   DSP_BGSND_ConfigMixer(KAL_TRUE, 7, BGSND_UL_PROCESS);
	...
	kal_trace( TRACE_FUNC,  DSP_BGSND_START_LEAVE);
}
/*===============  Ext BGSND interface ========= */

uint32 EXT_BGSND_init()
{
   kal_trace( TRACE_FUNC, EXT_BGSND_INIT_ENTER);
   DSP_BGSND_INIT();
   memset((void *)&Ext_BGSnd, 0, sizeof(EXT_BGSND_T));      
   // get audio ID
	Ext_BGSnd.aud_id = L1Audio_GetAudioID(); 
	Ext_BGSnd.skip_hisr = 0;
	kal_trace( TRACE_FUNC,  EXT_BGSND_INIT_LEAVE);
      ...
      if(!Ext_BGSnd.is_used[i]){
        kal_trace( TRACE_FUNC, EXT_BGSND_START_DEBUG2, i);
         ...
         if( 0 == Ext_BGSnd.num_src_used++){
           kal_trace( TRACE_FUNC, EXT_BGSND_START_SELECTED_SRC, i,  Ext_BGSnd.num_src_used);
        ...
        kal_trace( TRACE_FUNC, EXT_BGSND_START_DEBUG3, i);
      	break;
      }
     kal_trace( TRACE_FUNC, EXT_BGSND_START_DEBUG4, i);
   }   
  kal_trace( TRACE_FUNC, EXT_BGSND_START_DEBUG5, i);
   ASSERT(i < MAX_SIZE_EXT_BGSND_SRC);  
  kal_trace( TRACE_FUNC, EXT_BGSND_START_LEAVE);         
   return i;
}
...
{                               //flush buffer first, and then close it ; HISR will consume the buffer until it is empty. 
   int j;
  kal_trace( TRACE_FUNC, EXT_BGSND_FLUSH_ENTER);   
   ASSERT(Ext_BGSnd.is_used[id]);
   
   EXT_BGSND_SRC_T *pSrc = &Ext_BGSnd.src[id];
  kal_trace( TRACE_FUNC, EXT_BGSND_FLUSH_ID, id);
   ...
   for(j=0; j<MAX_NUM_BGSND_PROCESS; j++){  
      if(EXT_SRC_STATE_RUN == pSrc->state[j]){    
        ...
        kal_trace( TRACE_FUNC, EXT_BGSND_FLUSH_STATE, pSrc->state[j]);         
      }
   }
  kal_trace( TRACE_FUNC, EXT_BGSND_FLUSH_LEAVE);   
}
...
{
  kal_trace( TRACE_FUNC, EXT_BGSND_CLOSE_ENTER);     
   ASSERT(Ext_BGSnd.is_used[id]);
   EXT_BGSND_SRC_T *pSrc = &Ext_BGSnd.src[id]; 
  ...
  kal_trace( TRACE_FUNC, EXT_BGSND_CLOSE_SELECTED_SRC, id,  Ext_BGSnd.num_src_used);
  ...
  kal_trace( TRACE_FUNC, EXT_BGSND_CLOSE_LEAVE);     
}

void EXT_BGSND_DLHisr(void)
{
  kal_trace( TRACE_FUNC, EXT_BGSND_DLHISR_ENTER);
   EXT_BGSND_Hisr(BGSND_DL_PROCESS);
  kal_trace( TRACE_FUNC, EXT_BGSND_DLHISR_LEAVE);
}

void EXT_BGSND_ULHisr(void)
{
  kal_trace( TRACE_FUNC, EXT_BGSND_ULHISR_ENTER);
   EXT_BGSND_Hisr(BGSND_UL_PROCESS);
  kal_trace( TRACE_FUNC, EXT_BGSND_ULHISR_LEAVE);
}

/*static*/ int32 EXT_BGSND_GetDataCount(uint32 id, BGSND_PROCESS_TYPE type)
{
	int32 count = 0; 
	kal_trace( TRACE_FUNC,  EXT_BGSND_GETDATACOUNT_ENTER);
...
/*   for(j=0; j<MAX_BGSND_BUFFER_PROCESS_BIT; j++){
	...
	kal_trace( TRACE_FUNC,  EXT_BGSND_GETDATACOUNT_COUNT, count);
	kal_trace( TRACE_FUNC,  EXT_BGSND_GETDATACOUNT_LEAVE);
	return count;
}
...
void EXT_BGSND_Hisr(BGSND_PROCESS_TYPE type)
{
  ...
  kal_trace( TRACE_FUNC, EXT_BGSND_HISR_ENTER);
  kal_trace( TRACE_FUNC, EXT_BGSND_HISR_BGSND_PROCESS_TYPE, type);
   ...
   for(i=0; i<MAX_SIZE_EXT_BGSND_SRC; i++){         
      EXT_BGSND_SRC_T *pSrc = &Ext_BGSnd.src[i];   
      if(Ext_BGSnd.is_used[i]){
        kal_trace( TRACE_FUNC, EXT_BGSND_HISR_SELECTED_SRC, i, pSrc->state[type]);
         ...
         if(EXT_SRC_STATE_FLUSHING == pSrc->state[type]){ // FLUSHING --> FLUSHING_OVER            
            ...
            if( (0 == pSrc->len_WriteSrcBuffer_this_time) && (EXT_BGSND_GetDataCount(i, type) < BGSND_BUFFER_LEN) ){
              kal_trace( TRACE_FUNC, EXT_BGSND_HISR_EXT_SRC_STATE_FLUSHING_OVER, i, type); 
               ...
               if(EXT_SRC_STATE_FLUSHING_OVER == pSrc->state[1-type]){ // 1-type is just for the opposite.  DL <-> UL
                 kal_trace( TRACE_FUNC, EXT_BGSND_HISR_ENTER_SRC_OFFHDR, i); 
                  Ext_BGSnd.skip_hisr = 1;
                  pSrc->offHdr();   //FLUSHING_OVER --> STOPPING
                 kal_trace( TRACE_FUNC, EXT_BGSND_HISR_LEAVE_SRC_OFFHDR, i); 
                 kal_trace( TRACE_FUNC, EXT_BGSND_HISR_FLUSH_SRC_OFFHDR_COMPLETELY, i);            
               }else{//wait DL's flushing
                 kal_trace( TRACE_FUNC, EXT_BGSND_HISR_WAIT_SRC_HDR_TO_FLUSH_COMPLETELY, i);
               }
            }else{
              kal_trace( TRACE_FUNC, EXT_BGSND_HISR_FLUSHING, i, type, EXT_BGSND_GetDataCount(i, type));  
            }
         }else{
           kal_trace( TRACE_FUNC, EXT_BGSND_HISR_NOT_FLUSHING, i, type, pSrc->state[type]);  
         }
         

                   
      }else{
        kal_trace( TRACE_FUNC, EXT_BGSND_HISR_HDR_IS_NOT_USED, i);    
      }
      pSrc->len_WriteSrcBuffer_this_time = 0;
   }  
  kal_trace( TRACE_FUNC, EXT_BGSND_HISR_LEAVE);
}
...
void EXT_BGSND_Buf_Add(uint32 id, uint16* src_buf, int src_len, BGSND_PROCESS_TYPE type) // write into ext buffer
{
  ...
  kal_trace( TRACE_FUNC, BGSND_DSP_BGSND_BUF_ADD, id, src_len, type);
   ...
   { // DL    
      ...
      for(i=0; i<src_len; i++){//accumulate DSP_DLBuffer
         if( src_buf[i] < (0x1111111111111111 - dst_buf[i])){ //limiter
           ...
           kal_trace( TRACE_FUNC, BKGSND_DSP_BUF_ADD_BUF, i, src_buf[i], i, dst_buf[i]);  
         }
      }      
   }
}*/
...
void EXT_BGSND_WriteSrcBuffer(uint32 id, kal_uint8 *srcBuf, kal_int32 bufLen, BGSND_PROCESS_TYPE type)
{
   ...
   kal_trace( TRACE_FUNC, EXT_BGSND_WRITESRCBUFFER_ENTER);
   ...
   pSrc->len_WriteSrcBuffer_this_time = bufLen;
	...
	kal_trace( TRACE_FUNC,  EXT_BGSND_WRITESRCBUFFER_LEAVE);
}
...
void EXT_BGSND_WriteExtBuffer(uint32 id, int gain, BGSND_PROCESS_TYPE type) // ToDo
{
   ...
   uint16 *dataPtr;
   uint16 gainFactor;
  kal_trace( TRACE_FUNC, EXT_BGSND_WRITEEXTBUFFER_ENTER);   
  kal_trace( TRACE_FUNC, EXT_BGSND_WRITEEXTBUFFER_SELECTED_SRC, id, type);   
	ASSERT( type == BGSND_DL_PROCESS || type == BGSND_UL_PROCESS );
   EXT_BGSND_SRC_T *pSrc = &Ext_BGSnd.src[id];
  kal_trace( TRACE_FUNC, EXT_BGSND_WRITEEXTBUFFER_FSPH, pSrc->fSph[type], pSrc->gain[type]);      
  ...
  kal_trace( TRACE_FUNC, EXT_BGSND_WRITEEXTBUFFER_COUNT, count);  
   ...
   if(count > 0) {
	   ...
	   if (segment > 0) {
	      ...
	      if (pSrc->bufRead[type] >= pSrc->bufSize[type])
			...
			kal_trace( TRACE_FUNC,    EXT_BGSND_WRITEEXTBUFFER_FILLED_TO_DSP, 1, segment, pSrc->bufRead[type], pSrc->bufWrite[type]);
	   }
	   ...
	   if (segment > 0) {
	      ...
	      if (pSrc->bufRead[type] >= pSrc->bufSize[type])
			...
			kal_trace( TRACE_FUNC,   EXT_BGSND_WRITEEXTBUFFER_FILLED_TO_DSP, 2, segment, pSrc->bufRead[type], pSrc->bufWrite[type]);	
	   }		
	}  
   ...
   if (count > 0 && count < BGSND_BUFFER_LEN && EXT_SRC_STATE_RUN == pSrc->state[type]) {      		
      ...
      //   *toPtr++ = last_sample;     				
      //}	
     kal_trace( TRACE_FUNC,  EXT_BGSND_WRITEEXTBUFFER_FILLED_TO_DSP_WITH_LAST_SAMPLE, segment, last_sample);
      if(pSrc->fSph[type] && MAX_LEVEL_EXT_BGSND_GAIN!=gainFactor){
         for (i=0; i<segment; i++) {
            //*toPtr++ = *dataPtr++;     				
...
/*   {
      ...
      int i;
      for(i=0; i<10; i++){
        kal_trace( TRACE_FUNC, EXT_BGSND_WRITEEXTBUFFER, 
  ...
  kal_trace( TRACE_FUNC, EXT_BGSND_WRITEEXTBUFFER_LEAVE);    
}
...
int32 EXT_BGSND_GetFreeSpace(uint32 id, BGSND_PROCESS_TYPE type)
{
  ...
  kal_trace( TRACE_FUNC, EXT_BGSND_GETFREESPACE_ENTER); 
  kal_trace( TRACE_FUNC, EXT_BGSND_GETFREESPACE_INFO, pSrc->bufSize[type], pSrc->bufRead[type], pSrc->bufWrite[type]); 
   

   
  kal_trace( TRACE_FUNC, EXT_BGSND_GETFREESPACE_LEAVE);    
...
{
   EXT_BGSND_SRC_T *pSrc = &Ext_BGSnd.src[id];   
  kal_trace( TRACE_FUNC, EXT_BGSND_CONFIGMIXER_ENTER); 
   ASSERT(SNDGain>=0 && SNDGain<=MAX_LEVEL_EXT_BGSND_GAIN);
  kal_trace( TRACE_FUNC, EXT_BGSND_CONFIGMIXER_BSPHFLAG, bSPHFlag, SNDGain); 
   pSrc->fSph[type] = bSPHFlag;
   pSrc->gain[type] = MAX_LEVEL_EXT_BGSND_GAIN-SNDGain;
  kal_trace( TRACE_FUNC, EXT_BGSND_CONFIGMIXER_LEAVE);  
   /*
   SNDGain = 7 --> Volume >> 0
   SNDGain = 6 --> Volume >> 1
   SNDGain = 5 --> Volume >> 2
