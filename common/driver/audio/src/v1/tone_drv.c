...
static void dtmf_dsp_PlayOn( void *data )
{
   kal_trace( TRACE_FUNC,DTMF_DSP_PLAYON_ENTER);
   if(DTMF_DSP.uState == DTMF_DSP_STATE_IDLE)
   {
      return;
   }
...
//   }
   kal_trace( TRACE_FUNC,DTMF_DSP_PLAYON_LEAVE);
}
*/

static void dtmf_dsp_PlayOff( void *data )
{
   kal_trace( TRACE_FUNC,DTMF_DSP_PLAYOFF_ENTER);
   if(DTMF_DSP.uState == DTMF_DSP_STATE_IDLE)
   {
      return;
   }
...
//   }
   kal_trace( TRACE_FUNC,DTMF_DSP_PLAYOFF_LEAVE);
}

static void dtmf_dsp_Start()
{
   kal_trace( TRACE_FUNC,DTMF_DSP_START_ENTER);
   dtmf_dsp_StopTimer();
...
//   Data_Sync_Barrier();
   dtmf_dsp_SetTimer( DTMF_DSP.pCurrQtmf->on_duration, dtmf_dsp_PlayOff);
   kal_trace( TRACE_FUNC,DTMF_DSP_START_LEAVE);
}
...
{
#if 1
   kal_trace( TRACE_FUNC,DTMF_DSP_PLAY_ENTER);
   KT_StopAndWait();
   TONE_StopAndWait();
...
//      Data_Sync_Barrier();
   ...
   dtmf_dsp_Start();
   kal_trace( TRACE_FUNC,DTMF_DSP_PLAY_LEAVE);
#else
/* under construction !*/
/* under construction !*/
/* under construction !*/
