   ...
   if (NULL == callback_func) {
    ...
    for (idx = 0; idx < pool_size; idx++) {
        ...
        if (!IPC_IS_VALID_OBJECT(ntfy)) {
            IPC_INIT_OBJECT_BEGIN(ntfy, ipc_spinlock_g);

            ntfy->ntfy_id = idx;
            ntfy->callback_func= callback_func;
            ntfy->callback_context = callback_context;

            IPC_INIT_OBJECT_END(ntfy, ipc_spinlock_g);
            return ntfy;
        }
    }
    ...
    ipc_ntfy_t    *ntfy)
{
    ...
    IPC_DEINIT_OBJECT_BEGIN(ntfy, ipc_spinlock_g);
    if (ntfy) {
        IPC_DEINIT_OBJECT_END(ntfy, ipc_spinlock_g);
    } else {
    ...
    ipc_ntfy_t      *ntfy)
{
    ...
    IPC_W_LOCK_OBJECT(list, ipc_spinlock_g);
    if (list) {
        list->ntfys[list->count] = ntfy;
        list->count++;
        IPC_W_UNLOCK_OBJECT(list, ipc_spinlock_g);
    } else {
    ...
    kal_int32      ntfy_id)
{
    ...
    IPC_R_LOCK_OBJECT(ntfy, ipc_spinlock_g);
    if (ntfy) {
        ...
        IPC_R_UNLOCK_OBJECT(ntfy, ipc_spinlock_g);
