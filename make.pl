...
@actions = qw(errc_gen new update remake clean codegen emiclean emigen sysgen sys_auto_gen sys_mem_gen ckmemlayout drv_feature_check c,r c,u removecode scan check_scan check check_dep remake_dep update_dep clean_codegen slim_codegen slim_update genlog elfpatch cksysdrv nvram_auto_gen cust_menu_tree_check genmoduleinfo gendummylis gendummysym umts_gen asngen asngen_l4 asnregen slim_mcddll lte_mcddll eas_gen memquery cga mcddll_update);
@orgARGV = @ARGV;
...
while ($#ARGV != -1) {
  if ($ARGV[0] =~ /^(p|pl|pla|plat|platf|platfo|platfor|platform)=(\w+)/i) {
    ...
    $target_options .= " CCACHE_SIZE=$ccache_size"
  	...
  	# match <custom>(<flavor>)=>SUPERMAN29_DEMO(FLAVOR)
    if ($custom =~ /^(\w+)\((\w+)\)$/i) {
        $custom = $1;
        $flavor = $2;
...
if ((-e "label.ini") && (!-d "pcore/modem/lte_sec") && ($no_ltelib eq "FALSE") && ($no_ltecgen eq "FALSE") && ($flavor!~ /BASIC/i) &&
  ...
  if (-e "common/mtk_tools/USR/LTEUSR_Initial.pl") {
    $result = system("perl common/mtk_tools/USR/LTEUSR_Initial.pl $project_name $flavor $smart");
    exit 1 if ($result);
  } else {	
    die "[Error:] common/mtk_tools/USR/LTEUSR_Initial.pl is not exist!!\n";
  }
...
if ((-e "label.ini") && ($no_dsp eq "FALSE") && ($^O ne "MSWin32") &&
    (($action =~ /getusr/i) || ($action =~ /new/i) || ($action =~ /update/i) || ($action =~ /remake/i) || ($action =~ /codegen/i) || ($action =~ /c,u/i) || ($action =~/c,r/i))){
  if (-e "common/mtk_tools/USR/DSPUSR_Initial.pl") {
    $result = system("perl common/mtk_tools/USR/DSPUSR_Initial.pl $project_name $flavor $dsp_project $dsp_flavor");
  exit 1 if ($result);
  } else {
  	die "[Error:] common/mtk_tools/USR/DSPUSR_Initial.pl is not exist!!\n";
  }
...
if ((-e "label.ini") && (!-e "make/common/rel/hq/USR.txt")&&
    (($action =~ /getusr/i) || ($action =~ /new/i) || ($action =~ /update/i) || ($action =~ /remake/i) || ($action =~ /codegen/i) || ($action =~ /c,u/i) || ($action =~/c,r/i))){
#	$no_ltelib = "TRUE";
  if (-e "common/mtk_tools/USR/CBrUSR_Initial.pl") {
    $result = system("perl common/mtk_tools/USR/CBrUSR_Initial.pl $project_name $flavor $original_flavor $smart");
    exit 1 if ($result);
  } else {
  	die "[Error:] common/mtk_tools/USR/CBrUSR_Initial.pl is not exist!!\n";
  }
  die "[Error:] mtk_rel/$project_name/$flavor/MoDIS_lib is not exist, $project_name($flavor) can't build modis!!\n" if (($action =~ /_(modis)$/i) && (!-d "mtk_rel/$project_name/$flavor/MoDIS_lib"));
  die "[Error:] mtk_rel/$project_name/$flavor/UESim_lib is not exist, $project_name($flavor) can't build uesim!!\n" if (($action =~ /_(uesim)$/i) && (!-d "mtk_rel/$project_name/$flavor/UESim_lib"));
  die "[Error:] mtk_rel/$project_name/$flavor/lib is not exist, $project_name($flavor) can't build target!!\n" if (($action !~ /_(uesim|modis)$/i) && (!-d "mtk_rel/$project_name/$flavor/lib"));
...
if(-l "./common/tools/GCC/MIPS/4.9.2/linux") {
  #remove existing symbol link if link path is wrong
  $exist_symlink_path = readlink("./common/tools/GCC/MIPS/4.9.2/linux");
  if($exist_symlink_path ne "/mtkoss/imgtec/gcc-mips-mti-elf.2016.05-08/4.9.2/linux") {
    system("rm -f ./common/tools/GCC/MIPS/4.9.2/linux");
  }
}
if(!-l "./common/tools/GCC/MIPS/4.9.2/linux" && !-d "./common/tools/GCC/MIPS/4.9.2/linux" && -d "/mtkoss/imgtec/gcc-mips-mti-elf.2016.05-08/4.9.2/linux") {
  mkpath("./common/tools/GCC/MIPS/4.9.2") if(!-d "./common/tools/GCC/MIPS/4.9.2");
  system("ln -s /mtkoss/imgtec/gcc-mips-mti-elf.2016.05-08/4.9.2/linux ./common/tools/GCC/MIPS/4.9.2");
}
...
foreach my $action (@theAct) {
  ...
  if ($action eq "clean") {
    ...
    my $result = system("$makeCmd -f${makeFolder}custom_release.mak -r -R PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=$theMF\" FLAVOR=$flavor $target_options RELEASE_DIR_O=$relDir LEVEL=$level ckcr");    
    if($result){
      print "ckcr fail\n";
      exit 1;
    }
    ...
    $result = system("$makeCmd -f${makeFolder}custom_release.mak custom_release -r -R PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=$theMF\" FLAVOR=$flavor $target_options RELEASE_DIR_O=$relDir LEVEL=$level");
    &writeINI;
    exit 1 if ($result);
    exit 0;
    ...
    print "${makeCmd} -f${makeFolder}${myMF} -r -R CORE_ROOT=$core_root PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=$theMF\" FLAVOR=$flavor $target_options REMOVE_DIR=$remove_dir $action";
    system("${makeCmd} -f${makeFolder}${myMF} -r -R CORE_ROOT=$core_root PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=$theMF\" FLAVOR=$flavor $target_options REMOVE_DIR=$remove_dir $action");
	...
	if ($bypassMoDIS < 1) {
		...
		if ($modis_action eq "rel_modis") {
			if ($pureMoDIS == 1) {
				...
				$modis_result = system("$makeCmd -f${makeFolder}custom_release.mak -r -R PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=$theMF\" FLAVOR=$flavor $target_options RELEASE_DIR_O=$relDir LEVEL=$level $modis_option MODIS_CONFIG=FALSE base nvramautogen_rel files_list obj_list build_rel done");
			}
			if ($modis_result == 0) {
...
sub writeINI {
  ...
  if (($action ne "ckmake") && ($action ne "drv_feature_check")) {
    ...
    system("perl pcore/tools/init/echo.pl -n COMMAND=make \"@orgARGV\">>$targetFolder/build.log");
...
sub mbis_info_probe {
  if ($mbis_en eq "TRUE") {
    ...
    if (-e "make/common/rel/hq/USR.txt") 
    {
      ...
      {
        system("perl $mbis -i CODEBASE,LTE");
      }
      else
      {
...
sub mbis_end_probe {
  if ($mbis_en eq "TRUE") { 
    ...
    system("./pcore/mtk_tools/upload_to_mpdshare \"$logFolder/mbis/*_time.tmp\" \"/901.SM_MBIS_LOG/original_data/$upload_folder/\" 1>/dev/null 2>&1 &");
    system("./pcore/mtk_tools/upload_to_mpdshare \"$logFolder/mbis/*_info.log\" \"/901.SM_MBIS_LOG/original_data/$upload_folder/\" 1>/dev/null 2>&1 &");
  }
}
