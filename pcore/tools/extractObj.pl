...
if (!-e $ini_file) {
  die "$ini_file OR pcore\\tools\\$ini_file did NOT exist!\n" if (!-e "pcore\\tools\\$ini_file");
  $ini_file = "pcore\\tools\\$ini_file";
}
...
if ($custom_release eq "TRUE") {
  $dbfileprefix = "BPLGUInfoCustomAppSrcP_MT62";
} else {
  $dbfileprefix = "BPLGUInfoCustomApp_MT62";
}
...
if (!-e $extractObjExe) {
  die "$extractObjExe OR pcore\\tools\\$extractObjExe did NOT exist!\n" if (!-e "pcore\\tools\\$extractObjExe");
  $extractObjExe = "pcore\\tools\\$extractObjExe";
}
