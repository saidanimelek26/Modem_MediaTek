...
@mapDirs = (
 [ qw(MinGW_Slim pcore\tools\MinGW) ],
 [ qw(msys_Slim\1.0 pcore\tools\MSYS) ],
 [ qw(make make) ],
);

@mapFiles = (
 [ qw(Scripts\clean_obj.pl pcore\tools clean_obj.pl) ],
 [ qw(MinGW\bin\mingw32-make.exe pcore\tools make.exe) ],
 [ qw(MinGW\bin\objcopy.exe pcore\tools\MinGW\bin objcopy.exe) ],
 ...
 [ qw(make\verifyCus.mak make verifyCus.mak) ],
);

@srcs = qw(
# for AST (temporary)
\\\\mbjswgwt1501\\E\\home\\3rd_party
# end (for AST)
\\\\glbfs14\\WCP\\sw_releases\\3rd_party
\\\\mediatek.inc\\Taiwan\\WCP\\Public\\Public\\3rd_party
\\\\mszsfs03\\Public\\SW_Releases\\3rd_party
\\\\bjsfs05\\Public\\SW_Releases\\3rd_party
\\\\mtisfs03\\Public\\SW_Releases\\3rd_party
