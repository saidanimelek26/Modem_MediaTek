...
open(sortlib,">$ARGV[1]/~sortedLibs.tmp") || die "Cannot open $ARGV[1]/~sortedLibs.tmp: $!";
my $i = 0;
my $j = 0;
my %path = "";
my @file = "";
...
if($originalLine!=$sortedLine){
	print "The library may be lost after sorting libraries. Please compare $ARGV[0] and make/pcore/~sortedLibs.tmp.";
	exit(1);
}
