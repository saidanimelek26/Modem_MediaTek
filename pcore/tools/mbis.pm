...
sub mbisPostProcess{
    ...
    if ($mbis_enable)
    {
      ...
      if($^O eq "MSWin32")
      {
        if (!-d "pcore/modem/lte_sec")
        {
          $copy_result = system("\\\\script.gslb.mediatek.inc\\script\\wcpsm\\MBIS\\mtk-autoftp-mbis.exe $1 $transfer_file_name*.*");
          if ($copy_result != 0)
          {
              system("copy /y $1\\$transfer_file_name\_mbis_time.tmp $1\\$transfer_file_name\_mbis_time.log > nul");
              &notify_owner("$1\\$transfer_file_name\_mbis_info.log","$1\\$transfer_file_name\_mbis_time.tmp","$1\\$transfer_file_name\_mbis_time.log","$1\\mbis_updload.log");
          }
        }
        else
        {  # in LTE domain
