...
foreach my $inc (keys %incHash)
{
    ...
    system("perl $mcuPath\\pcore\\tools\\mmi_feature_check.pl $prjName $compiler $viaOption $inc $inc FALSE 0 $outputPath");
...
if (!$f_onlyGenDefaultMacro)
{
    foreach my $file (keys %resHash)
    {
        ...
        system("pcore\\tools\\python25\\python.exe pcore\\tools\\py\\prexml.py $resFile $resFile");
    }
