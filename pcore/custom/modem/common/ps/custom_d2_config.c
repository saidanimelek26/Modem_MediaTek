...
void custom_handle_data_retry(kal_uint32 sbp_id, apn_data_retry_info_t *apn_retry_info, kal_uint16 err_cause, kal_uint32 ps_id, rat_enum rat, kal_uint8 *plmn)
{
    switch (sbp_id) {
        case 129:
            { // KDDI
                if(apn_retry_info->apn_type & DDM_APN_TYPE_EMERGENCY) { 
                    if(apn_retry_info->retrytime_type != RETRY_TYPE_WITH_SUGGEST) { // ensuring backoff timer is not affected
                    ...
                    if(TCM_ESM_TIMER_TIMEOUT == err_cause) { 
                        if(apn_retry_info->retrytime_type != RETRY_TYPE_WITH_SUGGEST) { // ensuring backoff timer is not affected
                            ...
                            apn_retry_info->retrytime_type = RETRY_TYPE_WITH_SUGGEST;
                            apn_retry_info->retry_time = 55;
                            apn_retry_info->end_time = 55 + kal_ticks_to_secs(kal_get_systicks());		
                        }
                    }
                }
                break;	  
