...
BEGIN_TRACE_MAP(MOD_SMU)
   ...
   TRC_MSG(INFO_SMU_GET_UNLOCK_TIME_LEFT, "sml_GetUnlockTimeLeft: end time = %d, unlock time = %d")
    ...
    TRC_MSG(INFO_CRRST_ALLOWED_CARRIERS_PRIORITIZED, "CRRST: AllowedCarriersPrioritized=%d")
     ...
     TRC_MSG(INFO_TFN_LOAD, "TFN SIMLOCK: Load!")
     TRC_MSG(INFO_SML_TFN_EFUSE, "TFN SIMLOCK: efuse  %d")
     TRC_MSG(INFO_SML_COPY_DATA_FROM_TFN, "TFN data copied to SML obj")
     TRC_MSG(INFO_SML_PUT_DATA_TO_TFN_SUCCESS, "write to TFN nvram LID success")
     TRC_MSG(INFO_SML_PUT_DATA_TO_TFN_FAIL, "write to TFN nvram LID success")
     TRC_MSG(INFO_SML_COPY_DATA_TO_TFN, "copy Lock data to TFN object from SML")
