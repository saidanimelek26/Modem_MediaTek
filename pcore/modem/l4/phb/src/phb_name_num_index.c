...
static void phb_compare_by_tel_num_sig(
                ...
                pindex_struct *second_pos,
                compare_result_type *result);
static void phb_compare_by_tel_num_sig_for_wild_char(
                data_entry_table_type *external_data,
                num_sig_type *first,
                pindex_struct *second_pos,
                compare_result_type *result);
static void phb_compare_by_tel_num_sig_for_wild_char_ext(
                data_entry_table_type *external_data,
                num_sig_type *first,
                pindex_struct *second_pos,
                compare_result_type *result);
...
static kal_uint16 phb_search_one_by_one(
                    pindex_struct *a,
                    data_entry_table_type *external_data,
                    kal_uint16 start,
                    kal_uint16 length,
 ...
 *  KAL_TRUE if success, KAL_FALSE else.
 *****************************************************************************/
kal_bool phb_name_num_index_delete(name_num_index_type *name_num_index, kal_uint16 index, kal_uint8 *ext_index, kal_uint8 storage, phb_type_enum type)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_uint16 old_data_entry_pos, old_index;
    data_entry_struct *old_data_entry;
    kal_uint16 old_name_index = 0, old_num_index = 0;
    kal_uint16 i = 0;
    kal_uint16 index1 = index + 1;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    kal_trace(TRACE_INFO, FUNC_PHB_NAME_NUM_INDEX_DELETE, index);
    ...
    {
        old_data_entry_pos = name_num_index->data_entry_table.used_count;
        for (i = 0; i < name_num_index->data_entry_table.used_count; i++)
        {
            if (name_num_index->data_entry_table.table[i].record_index == index1 &&
                name_num_index->data_entry_table.table[i].storage == storage)
            {
                old_data_entry_pos = i;
                break;
            }
        }
        old_data_entry = &name_num_index->data_entry_table.table[old_data_entry_pos];
        
        old_name_index = name_num_index->name_index.used_count;        
        for (i = 0; i < name_num_index->name_index.used_count; i++)
        {
            if(name_num_index->name_index.table[i].position == old_data_entry_pos)
            {
                old_name_index = i;
                break;
            }
        ...
        old_num_index = name_num_index->num_index.used_count;
        for (i = 0; i < name_num_index->num_index.used_count; i++)
        {
            if(name_num_index->num_index.table[i].position == old_data_entry_pos)
            {
                old_num_index = i;            
                break;
            }
        ...
        if (name_num_index->num_index.slots > 0)
        {
            phb_pindex_delete(&name_num_index->num_index, old_num_index);
        }

        phb_pindex_delete(&name_num_index->name_index, old_name_index);
...
kal_uint16 phb_name_num_index_find_index_by_alpha_id(name_num_index_type *name_num_index, alpha_id_type *alpha_id)
{
    ...
    if (candidate < name_num_index->name_index.used_count)
    {
        ...
        else if ((candidate + 1) < name_num_index->name_index.used_count)
        {
            phb_compare_by_alpha_id(
                &name_num_index->data_entry_table,
                alpha_id,
                ...
                &result1);
            if (result1.same_count > result.same_count)      /* Wilson 2004-05-16 */
            {
                return candidate + 1;
            }
        }
 ...
 *  index to name_num_index->num_index
 *****************************************************************************/
kal_uint16 phb_name_num_index_find_index_by_num_sig_for_wild_char(
            name_num_index_type *name_num_index,
            kal_uint8 signature,
            kal_uint8 tel_num)
{
    ...
    candidate = phb_binary_search(
                    ...
                    (kal_uint16) (name_num_index->num_index.used_count - 1),
                    &num_sig,
                    (compare_func_type) phb_compare_by_tel_num_sig_for_wild_char);

    if ((candidate + 1) < name_num_index->name_index.used_count)
    {
        phb_compare_by_tel_num_sig_for_wild_char(
            &name_num_index->data_entry_table,
            &num_sig,
            &name_num_index->num_index.table[candidate],
            &result);
...
/*****************************************************************************
 * FUNCTION
 *  phb_name_num_index_find_index_one_by_one_for_wild_char
 * DESCRIPTION
 *  Search the index of first occurence of telephone number in
 *  name_num_index->num_index.
 * PARAMETERS
 ...
 *  index to name_num_index->num_index
 *****************************************************************************/
kal_uint16 phb_name_num_index_find_index_one_by_one_for_wild_char(
            ...
            kal_uint8 tel_num)
{
    ...
    candidate = phb_search_one_by_one(
                    name_num_index->num_index.table,
                    &name_num_index->data_entry_table,
                    search_from_index,
                    name_num_index->num_index.used_count,
                    &num_sig,
                    (compare_func_type) phb_compare_by_tel_num_sig_for_wild_char_ext);
                ...
                compare_result_type *result)
{
    ...
    if (result->distance == 0)
    {
        if (first->storage != external_data->table[second_pos->position].storage)
        {
          ...
          * of SIM
          */
            if (first->storage == PHB_STORAGE_NVRAM)
            {
                result->distance = -1;
            }
            else
                ...
                compare_result_type *result)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_uint8 first_tel_num, ext_tel_num;
    kal_uint8 first_num_sig, ext_num_sig;
    ...
    first_tel_num = first->tel_num;
    ext_tel_num = external_data->table[second_pos->position].tel_num;
    first_num_sig = first->signature;
    ext_num_sig = external_data->table[second_pos->position].tel_num_sig;

    if ((kal_uint8)(first_tel_num & 0xf0) == 0xf0 && ext_tel_num == 0x0d)
    {
        result->distance = 0;
    }
    else if ((kal_uint8)(first_tel_num & 0xf0) != 0xf0 && ext_tel_num == 0xdd)
    {
        result->distance = 0;
    }
    else if (first_tel_num == ext_tel_num ||
        ((kal_uint8)(ext_tel_num & 0xf0) == 0xd0 && (kal_uint8)(ext_tel_num & 0x0f) == (kal_uint8)(first_tel_num & 0x0f)) ||
        ((kal_uint8)(ext_tel_num & 0x0f) == 0x0d && (kal_uint8)(ext_tel_num & 0xf0) == (kal_uint8)(first_tel_num & 0xf0)))
    {
        if (first_num_sig > ext_num_sig)
        {
            result->distance = 1;
        }
        else if (first_num_sig < ext_num_sig)
        {
            result->distance = -1;
        }
        else
    ...
    else if ((kal_uint8)(ext_tel_num & 0xf0) == 0xd0)
    {
        if ((kal_uint8)(first_tel_num & 0x0f) > (kal_uint8)(ext_tel_num & 0x0f))
        {
            result->distance = 1;
        }
        else
    ...
    else if ((kal_uint8)(ext_tel_num & 0x0f) == 0x0d)
    {
        if ((kal_uint8)(first_tel_num & 0xf0) > (kal_uint8)(ext_tel_num & 0xf0))
        {
            result->distance = 1;
        }
        else
...
/*****************************************************************************
 * FUNCTION
 *  phb_compare_by_tel_num_sig_for_wild_char_ext
 * DESCRIPTION
 *  Compare two 2-digits telephone numbers. If they are equal, compare
 *  signature.
 * PARAMETERS
                ...
                compare_result_type *result)
{
    ...
    if ((kal_uint8)(first_tel_num & 0xf0) == 0xf0)
    {
        if ((first_tel_num == ext_tel_num) || (ext_tel_num == 0xfd))
        {
            result->distance = 0;
        }
        else
        {
            if (first_tel_num > ext_tel_num)
            {
                result->distance = 1;
            }
            else
    ...
    {
        if ((first_tel_num == ext_tel_num) || (ext_tel_num == 0xdd))
        {
            result->distance = 0;
        }
        else
        {
            if (((kal_uint8)(first_tel_num & 0xf0) == (kal_uint8)(ext_tel_num & 0xf0)) && 
                ((kal_uint8)(ext_tel_num & 0x0f) == 0x0d))
            {
                result->distance = 0;
            }
            else if (((kal_uint8)(first_tel_num & 0x0f) == (kal_uint8)(ext_tel_num & 0x0f)) && 
                ((kal_uint8)(ext_tel_num & 0xf0) == 0xd0))
            {
                result->distance = 0;
            }
            else
                ...
                compare_result_type *result)
{
    ...
    if (result->distance == 0)
    {
        if (external_data->table[first_pos->position].storage != external_data->table[second_pos->position].storage)
        {
          ...
          * of SIM
          */
            if (external_data->table[first_pos->position].storage != PHB_STORAGE_NVRAM)
            {
                result->distance = 1;
            }
            else
 ...
 *  first is greater than, equal to, or less than second.(?)
 *****************************************************************************/
void phb_compare_by_tel_num_sig_for_sort_for_wild_char(
        data_entry_table_type *external_data,
        pindex_struct *first_pos,
        pindex_struct *second_pos,
        compare_result_type *result)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_uint8 first_tel_num, second_tel_num;
    kal_uint8 first_num_sig, second_num_sig;
    ...
    first_tel_num = external_data->table[first_pos->position].tel_num;
    second_tel_num = external_data->table[second_pos->position].tel_num;
    first_num_sig = external_data->table[first_pos->position].tel_num_sig;
    second_num_sig = external_data->table[second_pos->position].tel_num_sig;

    if ((kal_uint8) (second_tel_num & 0x0f) == 0x0d)
    {
        result->distance = 0;
    }
    else if (first_tel_num == second_tel_num ||
        ((kal_uint8)(second_tel_num & 0xf0) == 0xd0 && (kal_uint8)(second_tel_num & 0x0f) == (kal_uint8)(first_tel_num & 0x0f)) ||
        ((kal_uint8)(second_tel_num & 0x0f) == 0x0d && (kal_uint8)(second_tel_num & 0xf0) == (kal_uint8)(first_tel_num & 0xf0)))
    {
        if (first_num_sig > second_num_sig)
        {
            result->distance = 1;
        }
        else if (first_num_sig < second_num_sig)
        {
            result->distance = -1;
        }
        else
    ...
    else if ((kal_uint8)(second_tel_num & 0xf0) == 0xd0)
    {
        if ((kal_uint8)(first_tel_num & 0x0f) > (kal_uint8)(second_tel_num & 0x0f))
        {
            result->distance = 1;
        }
        else if ((kal_uint8)(first_tel_num & 0x0f) == (kal_uint8)(second_tel_num & 0x0f))
        {
            result->distance = 0;
        }
        else
    ...
    else if ((kal_uint8)(second_tel_num & 0x0f) == 0x0d)
    {
        if ((kal_uint8)(first_tel_num & 0xf0) > (kal_uint8)(second_tel_num & 0xf0))
        {
            result->distance = 1;
        }
        else if ((kal_uint8)(first_tel_num & 0xf0) == (kal_uint8)(second_tel_num & 0xf0))
        {
            result->distance = 0;
        }
        else
...
/*****************************************************************************
 * FUNCTION
 *  phb_search_one_by_one
 * DESCRIPTION
 *  search one by one from first index
 *  
 *  Note:
 *  Since the array to be searched does not stores unique keys,
 *  nearest algorithm is ALWAYS applied!!
                    ...
                    compare_func_type compare)
{
    ...
    {
        (*compare) (external_data, key, &a[i], &result);
        if (result.distance == 0)
        {
            return i;            
        }
