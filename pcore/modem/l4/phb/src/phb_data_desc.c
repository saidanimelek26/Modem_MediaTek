...
void phb_desc_set_pbr_tbl(kal_uint8 count, kal_uint16 occurrence, data_desc_enum desc, kal_uint8 filetype, kal_uint8 iap_pos, kal_uint8 *path, kal_uint8 sfi)
{
    ...
    if (filetype == 2)
    {
        ...
        phb_ptr->phb_pbr_table[count][occurrence].free_bitmap = 
            free_bitmap_TYPE2_FILE[sim][(iap_pos * PHB_MAX_PBR_ENTRIES) + occurrence];	
    ...
    if (desc == DATA_DESC_ADN)
    {
        phb_ptr->phb_pbr_table[count][occurrence].free_count = 0;
        phb_ptr->phb_pbr_table[count][occurrence].free_bitmap = 
            free_bitmap_ADN[sim][occurrence];	
 ...
 *  void
 *****************************************************************************/
kal_bool phb_desc_get_file_path(sim_file_index_enum idx, kal_uint8 occurrence, kal_uint16 pbr_rec, kal_uint8 *path, kal_uint8* sfi)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
//        kal_mem_cpy(path, usim_ef_info[fold_id].full_path, 4);
		...
		{
			*sfi = entry->sfi;
			kal_trace(TRACE_INFO, INFO_PHB_SFI_VALUE, *sfi, idx);
		}
    }
    return KAL_TRUE;
}
...
void phb_data_desc_mark_free(data_desc_enum desc, kal_uint16 pbr_rec, kal_uint16 record_index, kal_uint16 cov_index, kal_bool is_free, kal_bool print_log)
{
    ...
    if (
        ...
        ((phb_util_is_usim(phb_which_sim_ex())) || (phb_util_is_csim(phb_which_sim_ex()))) &&
#else
         phb_util_is_usim(phb_which_sim_ex()) &&
#endif
...
kal_uint16 phb_data_desc_find_next_free(data_desc_enum type)
{
    ...
    if (
        ...
        ((phb_util_is_usim(phb_which_sim_ex())) || (phb_util_is_csim(phb_which_sim_ex()))) &&
         ...
         phb_util_is_usim(phb_which_sim_ex()) &&
#endif
    type == DATA_DESC_ADN && phb_ptr->data_desc[DATA_DESC_PBR].record_num)
    {
        kal_uint16 pbr_rec;
...
kal_bool phb_desc_index_check(data_desc_enum type, kal_uint16 record_index)
{
	...
	if (record_index != PHB_NULL_VALUE &&
			record_index > 0 && phb_desc_is_type2_file(type) && 
			record_index <= ((phb_data_desc_get_record_num(type) > TYPE2_FILE_MAX_ENTRIES_COUNT) ? TYPE2_FILE_MAX_ENTRIES_COUNT : phb_data_desc_get_record_num(type)))
	{
		return KAL_TRUE;
	}
