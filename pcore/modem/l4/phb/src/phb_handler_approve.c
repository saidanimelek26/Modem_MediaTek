...
void phb_approve_handler(ilm_struct *ilm_ptr, control_block_type *control_block)
{
    ...
    if (ilm_ptr != NULL)
    {
        ...
        if (l4cphb_approve_req->approval_type == APPROVAL_TYPE_NONE)
        {
            ...
            if (phb_approve_ecc(NULL, &l4cphb_approve_req->tel_number, NULL, &esc) == KAL_TRUE)
            {
                phb_approve_confirm(PHB_ERRNO_SUCCESS, PHB_ECC, l4cphb_approve_req->src_id, NULL, NULL, esc);
            }
            else
...
if (l4cphb_approve_cnf->src_id == (kal_uint8)MOD_CVAL)
{
  phb_send_ilm(MOD_CVAL, MSG_ID_L4CPHB_APPROVE_CNF, local_param_ptr, NULL);
}
else if (l4cphb_approve_cnf->src_id == (kal_uint8)MOD_CVAL_ATC)
{
  phb_send_ilm(MOD_CVAL_ATC, MSG_ID_L4CPHB_APPROVE_CNF, local_param_ptr, NULL);
}
...
void phb_update_nw_ecc_list_handler(ilm_struct *ilm_ptr, control_block_type *control_block)
{
    ...
    #ifdef __KOR_CUSTOMIZATION__
        ecc_custom_reset_nw_context(ecc_src);
    #endif /* __KOR_CUSTOMIZATION__ */
...
void phb_sim_error_ind_handler(local_para_struct *local_para_ptr, peer_buff_struct *peer_buff_ptr)
{
	...
	{
		case SIM_PLUG_OUT: // sim hot plug out case
		  {
			kal_trace(TRACE_INFO, INFO_PHB_SIM_ECC_RESET);
			ecc_custom_reset_sim_context(sim_ecc);
            phb_abort_data(phb_ptr, phb_which_sim_ex(), PHB_ERRNO_FAIL);
		  }
			break;
	
		case SIM_POWEROFF_BY_L4:  // sim switch case
			  kal_trace(TRACE_INFO, INFO_PHB_SIM_SWITCH);
			  
			  kal_trace(TRACE_INFO, INFO_PHB_SIM_SWITCH_POWEROFF_BY_L4, msg_ptr->is_dss_no_reset, msg_ptr->sim_slot_id);

	        #if defined (__DSS_NO_RESET__)
	          if (msg_ptr->is_dss_no_reset == KAL_TRUE)
	          {
				  phb_abort_data(phb_ptr, msg_ptr->sim_slot_id, PHB_ERRNO_SIM_SWITCH);
                  return;
	          }
	       
		   #else
		#ifdef __REL5__
		      if (sim_ecc == ECC_SRC_SIM1)
		      {
			  kal_trace(TRACE_INFO, INFO_PHB_SWAP_NW_ECC_LIST);
			  ecc_custom_swap_nw_context();
		      }
		#endif
		#endif
			break;
		default:
			kal_trace(TRACE_INFO, INFO_PHB_WRONG_CAUSE_IN_SIM_ERROR);
			break;
	}
}
