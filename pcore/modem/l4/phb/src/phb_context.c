...
kal_uint8 sim_switch_ps[MAX_SIM_NUM];
kal_uint8 bootup_sim[MAX_SIM_NUM];
#endif
/*****************************************************************************
 * FUNCTION
 *  phb_context_reset
...
void phb_context_reset()
{
    ...
    phb_ptr->is_cdma2000_supported = gmss_is_cdma2000_supported();
	if (phb_ptr->is_cdma2000_supported == KAL_TRUE)
	{
		kal_trace(TRACE_INFO, PHB_TRC_GMSS_CDMA_2000_SUPPORTED);
		phb_ptr->startup_state = PHB_STATE_NOT_IN_STARTUP;
        kal_trace(TRACE_INFO, INFO_PHB_CDMA_STARTUP_STATE, phb_ptr->startup_state);
	}
#else
    phb_ptr->is_cdma2000_supported = KAL_FALSE;
#endif
}
