            ...
            kal_uint8 *buffer)
{
    ...
    #if defined(__KSC5601_WRITE__)
    else if (istr->charset == CHARSET_KSC5601)
    {
        kal_uint16 ch;
...
// KOR MVNO Operator Support
#if defined(__KSC5601_READ__)
kal_bool isKSC5601(kal_uint8 *raw_data, kal_uint8 raw_data_len) {
    kal_int8   i   = 0;
    kal_int8   len = raw_data_len/2;
    kal_uint16 c   = 0;

    if(raw_data[0] == CODING_UCS2_80 || raw_data[0] == CODING_UCS2_81 || raw_data[0] == CODING_UCS2_82)
        return KAL_FALSE;

    for (; i < len; i++) {
        c = ((raw_data[(i*2)] & 0xff) << 8) + (raw_data[(i*2+1)] & 0xff);
...
kal_uint8 istring_decode_from_sim(kal_uint8 raw_data_len, kal_uint8 *raw_data, istring_type *decoded_str)
{
    ...
    // KOR MVNO Operator Support    
#if defined(__KSC5601_READ__)
    if (isKSC5601(raw_data, raw_data_len))
    {                            
        kal_uint8 ch1;
        kal_uint8 ch2;

        kal_sys_trace("KSC5601 Data");
    ...
    {
        kal_sys_trace("No KSC5601 Data");
#endif //__KSC5601_READ__
...
char *find_pinying_str_for_ucs2(kal_uint16 ucs2_code)
{
    ...
    /*----------------------------------------------------------------*/
    return NULL;
#if 0    // Coverity CID 89094 : Remove dead code
/* under construction !*/
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
#if defined(__CDMA2000_RAT__)
kal_bool phb_util_is_csim(kal_uint8 sim_num)
{
	kal_bool is_usim = phb_util_is_usim(sim_num);
	if(!is_usim && phb_ptr->is_cdma2000_supported == KAL_TRUE)
	{
	  l4c_csim_type_enum cdma_info;
			#if defined(__GEMINI__) || defined(GEMINI_PLUS) || defined(__SGLTE__)
		cdma_info = sim_get_cdma_info(phb_current_mod - MOD_PHB);
	#else
	   cdma_info = sim_get_cdma_info(0);
	#endif
		if ( cdma_info == L4C_CSIM_TYPE_IS_CSIM || cdma_info == L4C_CSIM_TYPE_IS_UIM_AND_CSIM) 
		{
			kal_trace(TRACE_INFO, PHB_TRC_CDMA_CSIM);
			return KAL_TRUE;/* For CSIM*/
...
kal_bool phb_util_is_usim_csim(kal_uint8 sim_num)
{
    if((phb_util_is_usim(sim_num) == KAL_TRUE) && (phb_ptr->is_cdma2000_supported == KAL_TRUE))
    {
        l4c_csim_type_enum cdma_info = sim_get_cdma_info(phb_which_sim_ex());
...
kal_bool phb_util_is_cdma_card(void)
{
    kal_uint8 curr_sim = phb_which_sim_ex();
    kal_bool is_usim = phb_util_is_usim(curr_sim);

    if (phb_ptr->is_cdma2000_supported == KAL_FALSE || is_usim)
    {
         return KAL_FALSE;
    }
    else
    {
        ...
        cdma_info = sim_get_cdma_info(0);
#endif
        if (cdma_info != L4C_CSIM_TYPE_NO_UIM_NO_CSIM) 
        {
            kal_trace(TRACE_INFO, PHB_TRC_CDMA_CARD);
            return KAL_TRUE;/* 0:uim, 1:csim, 2: no uim and no csim, 3: csim & uim*/
        }
        else
        {
            return KAL_FALSE;
...
extern c2k_service_state_enum cuim_service_table_query(c2k_service_id_enum service_id, kal_uint8 uim_index);
#endif /*__MTK_TARGET__ */
kal_bool phb_UIM_service_table_Query(kal_uint8 sst_id)
{
  if (phb_ptr->is_cdma2000_supported == KAL_FALSE)
  	{
  	 return KAL_FALSE;
  	}
  else
	  {
	  kal_uint8 uim_index = phb_which_sim_ex();
	  c2k_service_state_enum service_state;
#ifdef __MTK_TARGET__
	 service_state = cuim_service_table_query(sst_id, uim_index);
#else
	  service_state = C2K_SERVICE_SUPPORT;
#endif /*__MTK_TARGET__ */

	  if (service_state == C2K_SERVICE_SUPPORT)
	  {
	    kal_trace(TRACE_INFO, PHB_TRC_UIM_SST_QUERY_SUPPORTED, sst_id);
		return KAL_TRUE;
	  }
	  else
	  {
	  	kal_trace(TRACE_INFO, PHB_TRC_UIM_SST_QUERY_NOT_SUPPORTED, sst_id, uim_index, service_state);
		return KAL_FALSE;
	  }
...
#ifdef __MTK_TARGET__
extern kal_uint8 cuim_dn_type_quire(kal_uint8 uim_index);
#endif /*__MTK_TARGET__ */
sim_dn_enum phb_UIM_get_dial_mode(void)
{
	sim_dn_enum ret_value = TYPE_NONE;
	...
	else
	{
	  	kal_uint8 uim_index = phb_which_sim_ex();
	  #ifdef __MTK_TARGET__
	  	ret_value = (sim_dn_enum)(cuim_dn_type_quire(uim_index));
	  #endif
	  	kal_trace(TRACE_INFO, INFO_PHB_STARTUP_CDMA_MODE, ret_value, uim_index);
	  	return ret_value;
	}
}
#endif
...
kal_uint8 phb_map_sim(kal_uint8 sim)
{
	...
	if(bootup_sim[0] == bootup_sim[1])
	{
	   kal_trace(TRACE_ERROR, ERROR_PHB_ABNORMAL_CASE1_DSS_TSTS);
		return sim;
	}
	for (i = 0; i < MAX_SIM_NUM; i++)
	{
	  if(bootup_sim[sim] == sim_switch_ps[i])
	  {	    
		kal_trace(TRACE_INFO, INFO_PHB_DSS_TSTS_MAP_SIM, sim, i);
	    return i;
	  }
	}
	
	kal_trace(TRACE_ERROR, ERROR_PHB_ABNORMAL_CASE2_DSS_TSTS);
	return sim;
}
#endif

#if defined(__CDMA2000_RAT__)
kal_bool phb_try_csim_fdn(sim_file_index_enum file_idx)
{
    kal_bool csim_fdn_service_exist;
    kal_bool csim_fdn_enable;

    kal_bool usim_fdn_service_exist;
    kal_bool usim_fdn_enable;

    if (file_idx != FILE_U_FDN_IDX)
    {
        return KAL_FALSE;
    }

    csim_fdn_service_exist = phb_UIM_service_table_Query(C2K_SERVICE_FDN);
    csim_fdn_enable = (TYPE_FDN == phb_UIM_get_dial_mode()) ? KAL_TRUE : KAL_FALSE;

    usim_fdn_service_exist = PHB_SIM_SERVICE_TABLE_QUERY(SERVICE_U_FDN);
    usim_fdn_enable = (TYPE_FDN == l4csmu_get_dial_mode()) ? KAL_TRUE : KAL_FALSE;

    kal_trace(TRACE_INFO, INFO_PHB_TRY_CSIM_FDN, file_idx, usim_fdn_service_exist, usim_fdn_enable, csim_fdn_service_exist, csim_fdn_enable);

    if (phb_util_is_usim_csim(phb_which_sim_ex()) == KAL_TRUE)
    {
        if(sbp_query_md_feature_by_ps(SBP_IS_OP12, phb_which_sim_ex()) == KAL_TRUE)
        {
            if ((csim_fdn_service_exist == KAL_TRUE) && (csim_fdn_enable == KAL_TRUE))   
             return KAL_TRUE;
        }
        else
        {
            if (usim_fdn_service_exist == KAL_FALSE)
            {
                /* no USIM FDN */
                return KAL_TRUE;
            }
            else if ((usim_fdn_enable == KAL_FALSE) && (csim_fdn_service_exist == KAL_TRUE) && (csim_fdn_enable == KAL_TRUE))
            {
                /* USIM FDN & CSIM FDN, but only CSIM FDN enable */
                return KAL_TRUE;
            }
        }
    }
