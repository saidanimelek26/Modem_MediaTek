...
/* under construction !*/
	...
	{
	   phb_ptr->ln_data_desc[LN_DATA_DESC_LND].is_support = phb_UIM_service_table_Query(C2K_SERVICE_LND);
	}
	else
#endif
...
static void phb_startup_copy_ln(ilm_struct *ilm_ptr, control_block_type *control_block)
{
   ...
   //in idle status,
    if (control_block->proc_stage == COPY_LN_NONE)
    {
      //query if support LND EF file, if not, may need some error handling or it's impossiable not support when call this API
        kal_bool sst;
      	...
      	if(phb_util_is_cdma_card())
	{
	  sst = phb_UIM_service_table_Query(C2K_SERVICE_LND);
	}
	else
#endif
	 sst = PHB_SIM_SERVICE_TABLE_QUERY(SERVICE_LND);
        if (sst == KAL_TRUE)
        {
	...
	if(phb_util_is_cdma_card())
	{
	  control_block->res_msg_id = MSG_ID_UIM_FILE_INFO_CNF;
	}
	else
#endif
	...
	if(phb_util_is_cdma_card())
	{
          phb_send_cdma_ilm(MOD_UIM, MSG_ID_UIM_FILE_INFO_REQ, param_ptr, NULL);			
	}
	else
#endif
	...
	if(phb_util_is_cdma_card())
	{
          control_block->res_msg_id = MSG_ID_UIM_READ_CNF;
	}
	else
#endif
