...
#if defined(__CDMA2000_RAT__)
||
			(ilm_ptr->msg_id == MSG_ID_UIM_FILE_INFO_CNF) ||
#ifdef __SIM_SEARCH_RECORD_SUPPORT__ 
			(ilm_ptr->msg_id == MSG_ID_UIM_SEARCH_RECORD_CNF) ||
#endif
			(ilm_ptr->msg_id == MSG_ID_UIM_READ_CNF) || 
			(ilm_ptr->msg_id == MSG_ID_UIM_WRITE_CNF)
#endif
)
    {
...
/* under construction !*/
...
#if defined(__CDMA2000_RAT__)
			case MSG_ID_UIM_SEARCH_RECORD_CNF:
#endif
    ...
    {
        kal_trace(TRACE_INFO, INFO_PHB_STARTUP_STATE, phb_ptr->state);
#ifdef __CDMA2000_RAT__
	if (phb_util_is_cdma_card())
	{
		if(phb_ptr->startup_state == PHB_STATE_NOT_IN_STARTUP)
		{
          phb_ptr->startup_state = PHB_STATE_FIRST_L4C_STARTUP;
		}
        kal_trace(TRACE_INFO, INFO_PHB_CDMA_STARTUP_STATE, phb_ptr->startup_state);
	}
	if (phb_util_is_cdma_card() == KAL_FALSE)
#endif
        {
	...
	if (phb_util_is_cdma_card())
	{
        ...
        kal_trace(TRACE_INFO, INFO_PHB_CDMA_STARTUP_STATE, phb_ptr->startup_state);
		kal_trace(TRACE_INFO, INFO_PHB_WAIT_UIM_STARTUP);
		return;
	}
#endif
    	if ((phb_ptr->state != PHB_STATE_LN_NOT_READY) && (ilm_ptr != NULL))
    	{
    		kal_trace(TRACE_INFO, INFO_PHB_STARTUP_RESET_ALL_CB, phb_ptr->state);
    		phb_reset_all_control_blocks();
    	}
        phb_ptr->state = PHB_STATE_NOT_READY;
        kal_trace(TRACE_STATE, STATE_PHB_NOT_READY);
...
#ifdef __CDMA2000_RAT__
    else if(phb_util_is_cdma_card() && (ilm_ptr->msg_id == MSG_ID_UIM_PHB_STARTUP_REQ))
    {
	    kal_trace(TRACE_INFO, INFO_PHB_STARTUP_STATE, phb_ptr->state);
        ...
        if(phb_ptr->startup_state == PHB_STATE_NOT_IN_STARTUP)
		{
          phb_ptr->startup_state = PHB_STATE_FIRST_UIM_STARTUP;
		}
        kal_trace(TRACE_INFO, INFO_PHB_CDMA_STARTUP_STATE, phb_ptr->startup_state);
...
#ifdef __SAT__
#if defined(__CDMA2000_RAT__)
    else if(phb_util_is_cdma_card() && (ilm_ptr->msg_id == MSG_ID_UIM_UTK_FILE_CHANGE_IND))
    {
    ...
    {
        phb_sim_error_ind_handler(ilm_ptr->local_para_ptr, ilm_ptr->peer_buff_ptr);
    }
#if defined (__DSS_NO_RESET__)
	else if (ilm_ptr->msg_id == MSG_ID_L4C_ME_RECONFIGURE_REQ)
	{
	   if (MOD_PHB == ilm_ptr->dest_mod_id)
	   {
		 kal_uint8 ps;

	    for (ps = 0; ps < MAX_SIM_NUM; ps++)
	    {
	     kal_trace(TRACE_INFO, INFO_PHB_DSS_TSTS_INIT_PS, ps, sim_switch_ps[ps], ps, bootup_sim[ps]);
	    }
		
	   #if defined(__GEMINI__) || defined(GEMINI_PLUS) || defined(__SGLTE__)
		 kal_trace(TRACE_INFO, INFO_PHB_DSS_RECONFIG_REQ, phb_current_mod); 
	   #endif
	     phb_me_reconfig_handler(ilm_ptr->local_para_ptr, ilm_ptr->peer_buff_ptr);
	   }
	   else
	   {
	   	#if defined(__GEMINI__) || defined(GEMINI_PLUS) || defined(__SGLTE__)
...
kal_bool phb_init()
{
    ...
    for (; i < PHB_SIM_NUM; i++)
    {
	 ...
	 for (ps = 0; ps < MAX_SIM_NUM; ps++)
	{
          sim_switch_ps[ps] = l4c_gemini_get_actual_sim_id((sim_interface_enum)ps);
		  bootup_sim[ps] = sim_switch_ps[ps];
	  kal_trace(TRACE_INFO, INFO_PHB_DSS_TSTS_INIT_PS, ps, sim_switch_ps[ps]);
	}
	#endif
...
kal_bool phb_reset(void)
{
	...
	{
        kal_trace(TRACE_INFO, INFO_PHB_CDMA_STARTUP_STATE, phb_ptr->startup_state);
		if(phb_ptr->startup_state == PHB_STATE_FIRST_UIM_STARTUP)
		{
          kal_trace(TRACE_INFO, INFO_PHB_CDMA_NOT_RESET);
          return KAL_FALSE;
		}
	}
    #endif
