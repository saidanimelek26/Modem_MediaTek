...
typedef enum { 
  SMS_STATUS_START, // start of SMS over SGs
  SMS_STATUS_STOP, // end of SMS over SGs
  SMS_STATUS_NUM, 
} sms_status_enum;
...
typedef enum { 
  SMS_SEMAPHORE_TYPE_NONE = 0x00,
  SMS_SEMAPHORE_TYPE_CS   = 0x01,
  SMS_SEMAPHORE_TYPE_HPS  = 0x02,
  SMS_SEMAPHORE_TYPE_MAX  = 0xff 
} sms_semaphore_type_enum;


typedef enum { 
  CALL_TYPE_INDICATION_NORMAL, // the call is normal call
  CALL_TYPE_INDICATION_EMERGENCY, // the call is emergency call
  CALL_TYPE_INDICATION_EMERGENCY_FROM_FLIGHT, // the call is emergency call from flight mode
...
typedef enum
{
	...
	/*ECC CM Service rejected with cause for which CM_SERV_REQ should be retried*/
    MM_EVENT_CM_SERV_REJ_AVOID_PLMN_RETRY_CURRENT_RAT, /*Inform NWSEL to avoid current PLMN and try search over same RAT*/
    MM_EVENT_CM_SERV_REJ_AVOID_PLMN_NOT_RETRY_CURRENT_RAT /*Inform NWSEL to avoid current PLMN and try search over other available RAT*/
} mm_event_type_enum;
...
typedef enum
{
    ...
    MODE_SWITCH_OTHER_SIM_LOC_CHG = 0x06
