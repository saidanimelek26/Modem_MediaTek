...
} pam_backoff_timer_report_ind_struct;
...
#define MAX_NUMBER_OF_4G_CELL   (MAX_NUMBER_OF_FREQ * 5)  // For LTE, report 5 cell per freq as maximum 

#define MAX_NUMBER_OF_3G_CELL (MAX_NUMBER_OF_FREQ*3)      //For UMTS, report 3 cell per freq as maximum

#define MAX_NUMBER_OF_2G_CELL	(MAX_NUMBER_OF_FREQ)	// For GSM: 1 freq, 1 cell

typedef struct 
{
    kal_uint8    num_of_record;
    EARFCN       earfcn[MAX_NUMBER_OF_FREQ];
} freq_scan_4g_input_struct;
...
typedef struct 
{
    ...
    kal_int16    rsrq;
    kal_int16    reserved;
} freq_scan_4g_cell_struct;

typedef struct 
{
    kal_uint8                   num_of_record;
    freq_scan_4g_cell_struct    freq_scan_4g_cell[MAX_NUMBER_OF_4G_CELL];
} freq_scan_4g_output_struct;

typedef struct
{
    kal_uint8 num_of_record;
    kal_uint16 uarfcn[MAX_NUMBER_OF_FREQ];
}freq_scan_3g_input_struct;
...
typedef struct 
{
    ...
    kal_uint16   psc;
    kal_int16    rscp;
    kal_int16    EcNo;/*just FDD_3G report,TDS not report this para*/
} freq_scan_3g_cell_struct;

typedef struct
{
    kal_uint8 num_of_record;
    freq_scan_3g_cell_struct freq_scan_3g_cell[MAX_NUMBER_OF_3G_CELL];
}freq_scan_3g_output_struct;

typedef struct 
{
    kal_uint16   arfcn;
    kal_bool	 is_gsm_band_1900;
} freq_list_2g_struct;
...
typedef struct 
{
    ...
    kal_uint8    bsic;
    kal_bool	 is_gsm_band_1900;
} freq_scan_2g_cell_struct;

typedef struct
{
	kal_uint8	num_of_record;
	freq_list_2g_struct	freq_list_2g[MAX_NUMBER_OF_FREQ];
} freq_scan_2g_input_struct;

typedef struct 
{
    kal_uint8	num_of_record;
	freq_scan_2g_cell_struct	freq_scan_2g_cell[MAX_NUMBER_OF_2G_CELL];
} freq_scan_2g_output_struct;

typedef struct
{
    freq_scan_4g_input_struct freq_scan_4g_input;
    freq_scan_3g_input_struct freq_scan_3g_input;
    freq_scan_2g_input_struct freq_scan_2g_input;
} freq_scan_input_struct;

typedef struct
{
    freq_scan_4g_output_struct freq_scan_4g_output;
    freq_scan_3g_output_struct freq_scan_3g_output;
    freq_scan_2g_output_struct freq_scan_2g_output;
} freq_scan_output_struct;
